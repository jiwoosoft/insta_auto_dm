...
| Edge Function 배포 | `instagram-webhook` version 4, `test-match` 배포 완료 |
| Meta 실제 Webhook 연결 | Secret 등록과 Meta 대시보드 설정 필요 |
| 프론트엔드와 Supabase 실데이터 연결 | 규칙 관리와 테스트 매칭 연결 진행 중 |
...
두 함수 모두 `verify_jwt=false`로 배포되어 있습니다. `instagram-webhook`은 Meta Webhook 검증 요청을 받아야 하므로 JWT 검증을 끈 상태입니다. 현재 `instagram-webhook` version 4는 규칙 미매칭 메시지를 수신 로그에만 저장하고 자동 답변은 발송하지 않습니다.

Edge Function 원본 코드는 [supabase/functions/instagram-webhook/index.ts](supabase/functions/instagram-webhook/index.ts)와 [docs/edge-function-guide-v3.md](docs/edge-function-guide-v3.md)의 섹션 6-B를 기준으로 관리합니다.
...
규칙 생성, 수정, 삭제와 테스트 매칭을 실제 Supabase에 연결하려면 `.env.local`에 아래 값을 설정합니다. 현재 로컬 작업공간에는 `.env.local`이 생성되어 있고, 이 파일은 Git에 커밋되지 않습니다.

```text
VITE_SUPABASE_URL=https://rslfcjydihvginylmhrv.supabase.co
VITE_SUPABASE_PUBLISHABLE_KEY=Supabase publishable key
```

환경변수가 없으면 프론트엔드는 기존 로컬 JSON 목업으로 동작합니다.
...
3. 수신 로그와 발송 로그 화면을 Supabase 실데이터로 전환합니다.
4. 대시보드 집계 카드를 Supabase 실데이터로 전환합니다.
