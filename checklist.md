...
## 추가 체크리스트. 수신/발송 로그 실데이터 연결

- [x] 수신 로그와 발송 로그 화면 코드 확인.
- [x] Supabase `incoming_messages`, `outgoing_messages` 실제 데이터 확인.
- [x] 로그 조회용 Supabase REST 함수 추가.
- [x] 수신 로그 화면을 실데이터로 연결.
- [x] 발송 로그 화면을 실데이터로 연결.
- [x] 브라우저와 빌드 검증.
- [x] 작업 기록 커밋.

## 추가 체크리스트. 대시보드 정보갱신 점검

- [x] 대시보드 화면과 정보갱신 버튼 코드 확인.
- [x] Supabase 데이터 조회 함수와 대시보드 집계 로직 확인.
- [x] 정보갱신 클릭 시 실제 요청과 상태 변경이 일어나는지 재현.
- [x] 원인 수정.
- [x] 빌드와 가능한 브라우저 동작 검증.
- [x] 작업 기록 커밋.
